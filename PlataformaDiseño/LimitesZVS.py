def graficarLimitesZVS():
    ...
    for j in (range(9)):                    #Se ejecuta una vez por cada curva parametrica a graficar
        ...
        for i in (range(21)):   #Se ejecuta la funcion para completar los vectores
            ...
            if(j<7):            #Se ejecuta para todas las parametricas segun ganancias en continua reflejadas al primario.
                y[i] = Gef_var*phi_var*PI*(1-abs(phi_var)/180)/180  #Se guarda valor vector y
            elif(j==7):         #Se ejecuta para curva de limite ZVS del puente P1
                if(phi_var==-90):
                    y[i] = -10000
                elif(phi_var==90):
                    y[i] = 10000
            ...
            elif(j==8):         #Se ejecuta para curva de limite ZVS del puente P2
        ...
        if(j==0): #Selecciona el grafico segun la curva a graficar
            plt.plot(x,y,'k^-',linewidth=1.5,label=r'$Gef='+str(round(Gef_var,1))+'$')
        elif(j==1):
            plt.plot(x,y,'ks-',linewidth=1.5,label=r'$Gef='+str(round(Gef_var,1))+'$')
        elif(j==2):
            ...
            plt.plot(x,y,'ro-',linewidth=1.5,label=r'$Limite ZVS P1$')
        elif(j==8):
            plt.plot(x,y,'bs-',linewidth=1.5,label=r'$Limite ZVS P2$')
