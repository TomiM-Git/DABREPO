def generarParametricas3(Vi,n,L,Rl1,Rl2):
    import numpy as np
    import CalcularVo
    ...
    #Vectores listos para incluir datos experimentales.
...
#    minSQ_80ohm_200= [32.9,21.7]
    ...
    for x in range(2):  # Bucle para la realizacion de 2 gráficas.
        ...
        if(x==0):
            ...
            major_ticks_Vo=np.arange(0,525,25)    #Graficar de 0V a 525V cada 25V.
            #Agrego gráfico de recta horizontal con tensión de entrada para referencia.
            plt.plot(minSQ_frecs,Vi_ref,'ko-',linewidth=2.0,label="Máxima entrada: Vi,max = "+str(Vi)+"[V]")
            plt.plot(minSQ_frecs,Vo_max,'k^-',linewidth=2.0,label="Máxima salida: Vo,max = "+str(200)+"[V]")
        else:
            ...
            major_ticks_Vo=np.arange(0,275,25)    #Graficar de 0V a 275V cada 25V.
            #Agrego gráfico de recta horizontal con tensión de entrada para referencia.
            plt.plot(minSQ_frecs,Vi_ref,'ko-',linewidth=2.0,label="Máxima entrada: Vi,max = "+str(Vi)+"[V]")
            plt.plot(minSQ_frecs,Vo_max,'k^-',linewidth=2.0,label="Máxima salida: Vo,max = "+str(200)+"[V]")
        for j in (range(3)):
            ...
            if(j==0):   #Cada j corresponde a un incremento en la variable D_var.
                ...
                # Descomentar bloques en caso de incorporar nuevos datos experimentales.
...
#                    plt.plot(x,Vo_80ohm_50,'bo-',linewidth=2.0,label=r'$Real: \phi= '+str(D_var*180)+'$')
#                    #Finalmente se grafica la recta de tendencia de los datos experimentales.
#                    plt.plot(minSQ_frecs,minSQ_80ohm_50,'bs-',linewidth=2.0,label=r'$Tendencia: \phi= '+str(D_var*180)+'$')
#                elif(Rl==Rl2):
#                    plt.plot(x,Vo_40ohm_50,'bo-',linewidth=2.0,label=r'$Real: \phi= '+str(D_var*180)+'$')
#                    plt.plot(minSQ_frecs,minSQ_40ohm_50,'bs-',linewidth=2.0,label=r'$Tendencia: \phi= '+str(D_var*180)+'$')
            elif(j==1):     
                plt.plot(x,y,'g^-',linewidth=2.0,label="Curva teórica: \u03A6= "+str(D_var*180)+"°")
#                if(Rl==Rl1):
#                    plt.plot(x,Vo_80ohm_100,'go-',linewidth=2.0,label=r'$Real: \phi= '+str(D_var*180)+'$')
#                    plt.plot(minSQ_frecs,minSQ_80ohm_100,'gs-',linewidth=2.0,label=r'$Tendencia: \phi= '+str(D_var*180)+'$')
#                elif(Rl==Rl2):
#                    plt.plot(x,Vo_40ohm_100,'go-',linewidth=2.0,label=r'$Real: \phi= '+str(D_var*180)+'$')
#                    plt.plot(minSQ_frecs,minSQ_40ohm_100,'gs-',linewidth=2.0,label=r'$Tendencia: \phi= '+str(D_var*180)+'$')
            elif(j==2):
                plt.plot(x,y,'r^-',linewidth=2.0,label="Curva teórica: \u03A6= "+str(D_var*180)+"°")
#                if(Rl==Rl1):
#                    plt.plot(x,Vo_80ohm_200,'ro-',linewidth=2.0,label=r'$Real: \phi= '+str(D_var*180)+'$')
#                    plt.plot(minSQ_frecs,minSQ_80ohm_200,'rs-',linewidth=2.0,label=r'$\ Tendencia: \phi= '+str(D_var*180)+'$')
#                elif(Rl==Rl2):
#                    plt.plot(x,Vo_40ohm_200,'ro-',linewidth=2.0,label=r'$Real: \phi= '+str(D_var*180)+'$')
#                    plt.plot(minSQ_frecs,minSQ_40ohm_200,'rs-',linewidth=2.0,label=r'$\ Tendencia: \phi= '+str(D_var*180)+'$')
